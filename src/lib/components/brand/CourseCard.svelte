<script lang="ts">
	...
	import { Play, RotateCcw, CheckCircle, Layers, ArrowRight, Lock } from "@lucide/svelte";
	...
	function resolveImage(c: Course, override: string | undefined): string | undefined {
	...
	let CtaIcon = $derived(
		state === "completed" ? RotateCcw : Play,
	);
...
>
	...
	>
		...
		<div class="flex flex-1 flex-col gap-2 p-4">
			<h3 class="text-base font-semibold text-foreground leading-snug line-clamp-2">
				{course.title}
			</h3>
			...
			{:else if state === "in_progress"}
				<p class="text-[11px] text-muted-foreground tabular mt-auto">
					Module {(course.progress?.current_module ?? 0) + 1} · {Math.round(pct)}%
				</p>
			{:else}
				<p class="text-[11px] text-muted-foreground tabular mt-auto">
					Not started
