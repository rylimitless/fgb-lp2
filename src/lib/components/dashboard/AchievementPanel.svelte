...
<DashboardCard title="Progress / Achievements" class="h-full">
	<div class="grid min-h-[118px] grid-cols-[minmax(0,1.05fr)_minmax(105px,.95fr)] gap-2 p-2">
		<div class="flex flex-col gap-1">
			{#each displayItems as achievement, index}
				{@const progress = progressFor(achievement.label)}
				<button
					type="button"
					class="focus-premium flex min-h-8 items-center gap-2 rounded-md border px-2 py-1 text-left text-[8px] transition-colors {selected === index ? 'border-accent/55 bg-accent/10 text-foreground' : 'border-border bg-surface-1 text-muted-foreground hover:border-border-strong'}"
					onclick={() => (selected = index)}
					aria-pressed={selected === index}
				>
					<BadgeMedal tier={achievement.tier} size={22} />
					<span class="min-w-0 flex-1">
						<span class="flex items-start justify-between gap-1">
							...
							<span class="shrink-0 text-[7px] uppercase tracking-wide {progress.value === 100 ? 'text-success' : progress.value > 0 ? 'text-accent' : 'text-muted-foreground'}">{progress.label}</span>
						</span>
						<span class="mt-1 block h-1 overflow-hidden rounded-full bg-muted" aria-hidden="true">
							<span class="block h-full rounded-full {progress.value === 100 ? 'bg-success' : 'bg-accent'}" style:width={`${progress.value}%`}></span>
						</span>
					</span>
					{#if progress.value === 100}
						<CheckCircle2 class="size-3 shrink-0 text-success" />
					{:else if progress.value > 0}
						<CircleDashed class="size-3 shrink-0 text-accent" />
					{:else}
		...
		<div class="relative overflow-hidden rounded-md border border-border bg-surface-1 p-2.5">
			...
			<p class="mt-1 text-[9px] leading-relaxed text-muted-foreground">
			...
			<div class="mt-3 border-t border-border pt-2">
				<p class="text-[8px] uppercase tracking-wider text-muted-foreground">XP reward</p>
				<p class="text-base font-bold tabular text-accent">{xpReward} XP</p>
			</div>
